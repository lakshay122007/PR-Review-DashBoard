...
function Navbar({ user }) {
    ...
    return (
        <nav style={{ fontFamily: "'DM Sans', sans-serif" }}
            className="flex items-center justify-between px-4 md:px-6 py-3 border-b border-white/[0.06] bg-[#080C14]/90 backdrop-blur-md sticky top-0 z-50">
        ...
        {user && (
            ...
            <div className="flex items-center gap-2 bg-white/[0.04] border border-white/[0.07] rounded-xl px-3 py-1.5">
                ...
                <span className="text-xs text-gray-300 hidden md:block">{user.login}</span>
            </div>
